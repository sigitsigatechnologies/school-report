...
class ProjectsResource extends Resource
{
    ...
    //                     // kalau user punya role guru
    //                     if ($user->hasRole('guru')) {
    //                         $guru = $user->guru;
    //                         $classroomIds = $guru->classrooms->pluck('id');

    //                         return StudentClassroom::with(['academicYear', 'classroom'])
    ...
    //                     // fallback kalau bukan guru (misalnya admin), tampilkan semua kelas aktif
    //                     return StudentClassroom::with(['academicYear', 'classroom'])
    //                         ->whereHas('academicYear', fn($q) => $q->where('is_active', true))
    //                         ->get()
    //                         ->mapWithKeys(function ($item) {
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                ...
                // Ambil langsung dari deskripsi detail
                Select::make('project_description_detail_id')
                    ->label('Pilih Judul Proyek dari Deskripsi')
                    ->options(function () {
                        $user = auth()->user();
                        ...
                        if (!$user->hasRole('guru')) {
                            return ProjectDescriptionDetails::with('header')->get()->pluck('title', 'id');
                        }

                        $classroomIds = $user->guru->classrooms->pluck('id');

                        return ProjectDescriptionDetails::whereHas(
                            'header',
                            fn($q) => $q->whereIn('classroom_id', $classroomIds)
                        )->pluck('title', 'id');
                    })
                    ->searchable()
                    ->required()
                    ->reactive()
                    ->afterStateUpdated(function (callable $set, $state) {
                        $detail = ProjectDescriptionDetails::with('header.classroom')->find($state);

                        $set('fase', $detail?->header?->fase);
                        $set('classroom', $detail?->header?->classroom?->name);
                    })
                    ->afterStateHydrated(function (callable $get, callable $set) {
                        $detail = ProjectDescriptionDetails::with('header.classroom')
                ...
                Repeater::make('projectDetails')
                    ...
                    ->schema([
                        Grid::make(12)->schema([
                            ...
                            Select::make('element_id')
                                ->label('Elemen')
                                ->options(fn(callable $get) => Elements::where('dimension_id', $get('dimension_id'))->pluck('name', 'id'))
                                ->reactive()
                                ->afterStateUpdated(fn($state, callable $set) => $set('sub_element_id', null))
                                ->required()
                                ->columnSpan(3),
                            ...
                            Select::make('sub_element_id')
                                ->label('Sub Elemen')
                                ->options(fn(callable $get) => SubElements::where('element_id', $get('element_id'))->pluck('name', 'id'))
                                ->reactive()
                                ->afterStateUpdated(fn($state, callable $set) => $set('capaian_fase_id', null))
                                ->required()
                                ->columnSpan(3),
                            ...
                            Select::make('capaian_fase_id')
                                ->label('Capaian Fase')
                                ->options(fn(callable $get) => CapaianFase::where('sub_element_id', $get('sub_element_id'))->pluck('description', 'id'))
                                ->required()
                                ->columnSpan(3),
                        ]),
                    ])
